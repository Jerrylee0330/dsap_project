...
| 比較維度 | **本專案 (微積分計算機)** | **Wolfram Alpha / Gemini** | **Photomath** |
| :--- | :--- | :--- | :--- |
| **運作環境** | **完全離線**，適合台大校園等網路不穩場域。 | 需連網，依賴雲端伺服器。 | 需連網。 |
| **技術架構** | AST 符號運算 + 數值分析 Fallback。 | 強大符號資料庫（黑盒模型）。 | 影像辨識 (OCR) + 解答資料庫。 |
| **輸入方式** | 專屬 GUI 按鈕與鍵盤輸入，精確度高。 | 純文字輸入，常有語法解析錯誤。 | 拍照掃描，受光線與字跡影響。亦可透過內建鍵盤輸入。 |
