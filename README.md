...
### 目前進度
成功實作 Lexer 與 Parser：利用 Shunting-yard Algorithm 將人類可讀的數學算式（中序式）精準轉化為 AST（抽象語法樹）。
建立符號微分引擎 (Symbolic Differentiation)：透過遞迴走訪 AST，成功實作涵蓋多項式、指數、對數、三角函數、反三角函數以及連鎖律 (Chain Rule) 的微分運算。
開發代數化簡模組 (Algebraic Simplification)：能在微分後自動進行常數摺疊 (Constant Folding，如 $\ln(e) = 1$)、同類項合併，以及消除冗餘的 * 1 或 + 0，大幅提升輸出算式的可讀性。

### 遇到的困難


### 下一步計畫
繼續利用 Shunting-yard Algorithm 完成積分的運算。
套用 Recursive Descent 方法計算微分與積分，並比較兩者在記憶體使用、運行時間等的差異。


### 與課程的關聯
在將算式轉為後序式時，利用stack建立。
把資料用抽象語法樹建立。
在計算微分時，透過遍歷樹的方式，並建立新的節點。
