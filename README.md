...
### 下一步計畫
擴充積分功能： 預計先實作基於 AST 的「數值積分 (Numerical Integration)」，透過黎曼和等方法計算定積分面積；後續再挑戰難度較高的「符號積分 (Symbolic Integration)」，讓引擎具備尋找反導數 (Antiderivative) 的能力。

解析演算法架構重構與效能評估： 著手開發 Recursive Descent Parser，取代現有的 Shunting-yard Parser 以支援更複雜的語法（如多參數函數 integral(f, a, b)）。同時，將針對兩種解析演算法在 AST 建構速度、記憶體消耗及擴充性上進行量化分析與比較。
